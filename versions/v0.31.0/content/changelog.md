...
- detect if the host is running on battery and cancel an action depending on how much battery is left - see [schedule section]({{% relref "/schedules/configuration/index.html#schedule-ignore-on-battery" %}})
...
- `copy` command now has `run-before`, `run-after`, `run-after-fail` and `run-finally` targets. Also more targets are now available for `backup` and for a profile. [See the documentation]({{% relref "/configuration/run_hooks/index.html" %}}).
